...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  PersonalData? _effectiveProfileForSettings({
    required PersonalData? dbPersonalData,
    required PersonalData? draftPersonalData,
    required User? authUser,
  }) {
    // Settings should primarily reflect DB-backed profile, per backend contract.
    final base = dbPersonalData ?? draftPersonalData;

    if (base == null && authUser == null) return null;

    final fallbackMobile = (authUser != null &&
            authUser.email.endsWith('@phone.local') &&
            authUser.email.contains('@'))
        ? authUser.email.split('@').first
        : null;

    return PersonalData(
      nameAsPerAadhaar:
          _nonEmpty(base?.nameAsPerAadhaar) ?? _nonEmpty(authUser?.name),
      mobileNumber:
          _nonEmpty(base?.mobileNumber) ?? _nonEmpty(fallbackMobile),
      personalEmailId:
          _nonEmpty(base?.personalEmailId) ?? _nonEmpty(authUser?.email),
      dateOfBirth: base?.dateOfBirth,
      panNo: _nonEmpty(base?.panNo),
      residenceAddress: _nonEmpty(base?.residenceAddress),
      occupation: _nonEmpty(base?.occupation),
    );
  }
            ...
            children: [
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    // Profile Section
                    Consumer2<SubmissionProvider, AuthProvider>(
                      builder: (context, submissionProvider, authProvider, _) {
                        final draftPersonalData =
                            submissionProvider.submission.personalData;
                        final dbPersonalData =
                            authProvider.profilePersonalData;
                        final authUser = authProvider.user;

                        final effectivePersonalData =
                            _effectiveProfileForSettings(
                          dbPersonalData: dbPersonalData,
                          draftPersonalData: draftPersonalData,
                          authUser: authUser,
                        );
